...
**/
dc.errorRowChart = function (parent, chartGroup) {
    ...
    function calculateAxisScale() {
        if (!_x || _elasticX) {
            var extent = d3.extent(_rowData, _chart.cappedUpperValueAccessor);
            if (extent[0] > 0) {
                extent[0] = 0;
            }
    ...
    function createElements(rows) {
        ...
        //Error bars
        rowEnter.append('rect').attr('width', 0).attr('class', 'error-bar');
    ...
    function updateElements(rows) {
        ...
        //Error bars
        var errorBarRect = rows.select('rect.error-bar')
        ...
        //Set width and y for the error bars
        dc.transition(errorBarRect, _chart.transitionDuration())
            .attr('width', function (d) {
                return Math.abs(rootValue() - _x(d.value.upper - d.value.lower));
            })
            .attr('x', function (d) {
                return Math.abs(_x(d.value.lower));
            })
            .attr('y', function (d) {
                return Math.abs((height - d3.select(this).attr('height')) / 2);
            });
    ...
    **/
    ...
    _chart.cappedUpperValueAccessor = function (d, i) {
        if (d.value.upper) {
